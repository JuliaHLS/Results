...
static void toggleClock(Vtop* dut, VerilatedVcdC* tfp) {
...
public:
        ...
        switch(state) 
        {
            case IDLE: 
            {
                ...
                if (state == AXI_state::IDLE && dut->m_axis_mem0_req_tvalid) {
                    ...
                    req = std::make_unique<write_req>(msg, 0);
            ...
            case WRITING:
            {
                ...
                if(delay_ctr++ == DELAY) {
                    std::cout << "Reading 0x" << req->addr << ", data: " << mem[req->addr] << std::endl;
                    auto data = mem[req->addr];
    ...
    // void set_data(std::vector<std::pair<uint64_t, uint64_t>> data) {
...
private:
    ...
    std::array<long, 9> mem = {1,2,3,4,5,6,7,8,9};
};
        ...
        switch(state) 
        {
            case IDLE: 
            {
                ...
                if (state == AXI_state::IDLE && dut->m_axis_mem2_store_tvalid) {
                    ...
                    // std::cout << "Got MSG: " << msg[1] << ", " << msg[1] << ", " << msg[0] << std::endl;
                    std::cout << "got msg " << msg << std::endl;
                    // std::cout << "got msg
                    auto top2 = static_cast<uint64_t>(msg) >> 32; 

                    // uint64_t low64 = (static_cast<uint64_t>(msg[1]) << 32)
                                   // |  static_cast<uint64_t>(msg[0]);
                    auto l = msg << 32;
                    auto l2 = msg >> 32;
                    uint64_t low64 = static_cast<uint64_t>(l2);
                    std::cout << "GOT ADDR " << top2 << ", with DATA: " << low64 << std::endl;


                    wreq = std::make_unique<write_req>(top2, low64);
            ...
            case WRITING:
            {
                ...
                if(delay_ctr++ == DELAY) {
                    std::cout << "Writing 0x" << wreq->addr << ", data: 0x" << wreq->data << std::endl;
                    mem[wreq->addr] = wreq->data;
                    state = AXI_state::DONE;
                }
...
private:
    ...
    AXI_state state;
    std::unique_ptr<write_req> req;
    std::unique_ptr<write_req> wreq;


    Vtop *dut;  

    std::array<long, 4> mem;
};
...
class ModuleDriver{
public:
    ModuleDriver(Vtop* _dut, VerilatedVcdC* _tfp) {
        dut = _dut;
        tfp = _tfp;
